...
from schdlr import task as base_task
...
def check_scheduler_state(schdlr, exp_state,
    ...
    exp_tasks_stat = {'in_queue': exp_tasks_in_queue,
                      ...
                      'done': exp_tasks_done}
    act_task_state = schdlr.tasks_stat()
    for k, v in act_task_state.items():
        if isinstance(v, list):
            act_task_state[k] = len(v)
...
def test_one_worker_one_task_pass_added_before_start():
    schdlr = scheduler.Scheduler(1)
    task1 = task_lib.FuncTask(foo, (1,), {'to_sleep': 0.3, })
...
def test_multiple_task_order_same_priority():
    ...
    task1 = task_lib.FuncTask(foo, (1,), {'to_sleep': 0.3, })
    task2 = task_lib.FuncTask(foo, (1,), {'to_sleep': 0.3, })
    task3 = task_lib.FuncTask(foo, (1,), {'to_sleep': 0.3, })
    ...
    assert schdlr.tasks_stat()["done"] == [task1, task2, task3]
...
def test_multiple_task_order_different_priority():
    ...
    schdlr.add_task(task1, priority=base_task.PRIORITY_LOW)
    schdlr.add_task(task2, priority=base_task.PRIORITY_MID)
    schdlr.add_task(task3, priority=base_task.PRIORITY_HI)
    ...
    check_scheduler_state(schdlr, scheduler.STATE_STOPPED, 0, 0, 3, None)
    compare_task_list(schdlr.tasks_stat()["done"],
                      [task3, task2, task1])
...
def test_multiple_task_order_different_priority_2():
    ...
    task2 = task_lib.FuncTask(foo, (1,), {'to_sleep': 0.3, })
    task3 = task_lib.FuncTask(foo, (1,), {'to_sleep': 0.3, })
    task4 = task_lib.FuncTask(foo, (1,), {'to_sleep': 0.3, })

    schdlr.add_task(task1, priority=base_task.PRIORITY_MID)
    schdlr.add_task(task2, priority=base_task.PRIORITY_MID)
    schdlr.add_task(task3, priority=base_task.PRIORITY_MID)

    schdlr.start()
    time.sleep(0.1)
    schdlr.add_task(task4, priority=base_task.PRIORITY_HI)
    schdlr.stop()
...
def test_overdue_tasks():
    ...
    task1 = task_lib.FuncTask(foo, (1,), {'to_sleep': 1, })
    task2 = task_lib.FuncTask(foo, (1,), {'to_sleep': 1, })
    task3 = task_lib.FuncTask(foo, (1,), {'to_sleep': 1, })

    schdlr.start()
    schdlr.add_task(task1, priority=base_task.PRIORITY_MID, timeout=0.1)
    time.sleep(0.2)
    assert len(schdlr.overdue_tasks) == 1

    schdlr.add_task(task2, priority=base_task.PRIORITY_MID, timeout=0.1)
    time.sleep(0.2)
    assert len(schdlr.overdue_tasks) == 2

    schdlr.add_task(task3, priority=base_task.PRIORITY_MID, timeout=0.1)
    time.sleep(0.2)
